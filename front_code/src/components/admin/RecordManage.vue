  ...
  <div class="container">
    <Form ref="formInline"  inline>
      <FormItem prop="borrowRecord">
        <Input type="text" v-model="formInline.query" placeholder="借阅者学号">
    ...
    <Table border :columns="columns7" :data="data6">
      ...
      <template slot-scope="{ row, index }" slot="action">
        ...
        <Button type="warning" size="small" v-if="row.status == 2" @click="callreback(row)">催还</Button>
      </template>
    </Table>
...
<script type="es6">
  export default {
    ...
    methods: {
      ...
      request (currentPage){
         this.$axios.get(`/admin/borrowrecord/${this.currPage}/${this.pageSize}`, {params: { query: this.formInline.query }}).then(res=>{
