  ...
  <div class="container" style="padding-top:5px;">
    <Form ref="formInline"  inline>
      <FormItem prop="borrowRecord">
        <Input type="text" v-model="formInline.query" placeholder="订单编号/学号">
    ...
    <Table border :columns="columns7" :data="data6">
      <template slot-scope="{ row }" slot="number">
        <Icon type="ios-book" size="14"></Icon>{{row.number}}
      </template>
      ...
      <template slot-scope="{ row }" slot="type">
        <Tag color="primary" v-if="row.type == 1">支付宝</Tag>
        <Tag color="success" v-if="row.type == 2">微信</Tag>
      </template>
...
<script type="es6">
  export default {
    ...
    methods: {
      ...
      request (currentPage){
         this.$axios.get(`/admin/order/${this.currPage}/${this.pageSize}`, {params: { query: this.formInline.query }}).then(res=>{
