  ...
  export default {
    ...
    methods: {
      ...
      remove (row) {
        ...
        const url = '/admin/talbum/' + ( flag ? 'down/' : 'up/') + row.aid; 
      ...
      request (currentPage){
         this.$axios.get(`/admin/talbum/${this.currPage}/${this.pageSize}`, {params: { query: this.formInline.query }}).then(res=>{
          this.total = res.total
          this.data6 = res.rows
        })
      },
      getCList() {
        this.$axios.get(`/admin/booktype/-1/-1`).then(res=>{
          this.clist = res.rows
        })
      },
      ...
      ok (name) {
        this.$axios.post('/admin/talbum', this.formItem2).then(res=>{
