  ...
  <div class="container" style="padding-top:5px;">
    ...
    <Table border :columns="columns7" :data="data6">
      ...
      <template slot-scope="{ row, index }" slot="action">
        <Button color="primary" @click="show(index)"  type="primary" size="small" v-if="row.status == 1 || row.status == 3">查看</Button>
        <Button type="warning" @click="reback(row)" size="small" v-else>归还</Button>
      </template>
    </Table>
  ...
  export default {
    ...
    methods: {
      ...
      reback (row) {
        ...
        this.$axios.put(`/api/reader/reback/${row.bid}`).then(res => {
          this.$Message.success('归还成功')
          this.request()
        })
      },
      request (currentPage){
         this.$axios.get(`/api/brecord/${this.currPage}/${this.pageSize}`, {params: { query: this.formInline.query }}).then(res=>{
